...
export const useItems = () => {
    ...
    // ❌ ГЛАВНАЯ ПРОБЛЕМА: Эта функция вызывает ререндер ВСЕГО массива items
    // при изменении ОДНОГО поля в ОДНОМ элементе
    const handleItemChange = useCallback((index: number, field: keyof Item, value: string): void => {
        setItems(prevItems => {
            ...
            // ❌ ПРОБЛЕМА 1: Создаем новый массив - все ItemRow ререндерятся
            const newItems = [...prevItems];
            const updatedItem = { ...currentItem, [field]: value };

            // ❌ ПРОБЛЕМА 2: calculateItemValues вызывается при каждом изменении
            newItems[index] = calculateItemValues(updatedItem);

            // ❌ ПРОБЛЕМА 3: setValidationErrors внутри setItems - двойной рендер
