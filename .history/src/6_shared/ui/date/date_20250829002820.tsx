...
interface DataProps {
    ...
    errors?: string;
    showError?: boolean;
    range?: boolean; // <<< новое свойство
    onChange: (
        value:
            | { date: Date; dateStr: string } // для одиночной даты
            | { start?: Date; end?: Date; dateStr: string } // для диапазона
    ) => void;
}
    ...
    const handleChange = (selectedDates: Date[], dateStr: string) => {
        if (range) {
            onChange({
                ...
                dateStr, // "2025-08-01 to 2025-08-28"
            });
        } else {
            onChange({
    ...
    return (
        <div>
          ...
          dark:focus:ring-blue-500 dark:focus:border-blue-500 dark:bg-gray-800 ${errors ? "dark:border-red-500 border-red-300" : "border-gray-300"
                ...
                options={{
                    mode: range ? "range" : "single", // <<< переключаем режим
