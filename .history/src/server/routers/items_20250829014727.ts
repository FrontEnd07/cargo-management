    ...
    getItemsProduct: protectedProcedure
        ...
        .query(async ({ input, ctx }) => {
            ...
            if (dateFrom || dateTo) {
                ...
                if (dateFrom) {
                    // Используем Prisma функцию для сравнения только даты
                    whereConditions.document.date.gte = new Date(dateFrom);
                }

                if (dateTo) {
                    whereConditions.document.date.lte = new Date(dateTo);
                }
            }

            // Фильтр по клиенту
            if (customerId) {
                whereConditions.customerCode = customerId;
            }
